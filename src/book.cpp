...
void Book_Operation::Import(int Quantity) {
    int p = Head.head;
    int size;
    while (p != -1) {
        size = link_[p].size;
    ...
    while (p != -1) {
        ...
        if (string_cmp(current_Book.ISBN, bloc_[0].ISBN, current_Book.ISBN_len, bloc_[0].ISBN_len) != -1 &&
            ...
            while (l <= r) {
                ...
                if (val == 0) {
                    if (type == 1) {
                        std::strcpy(bloc_[mid].ISBN, info.c_str());
                        bloc_[mid].ISBN_len = static_cast<int>(info.size());
                    }
                    if (type == 2) {
...
void Book_Operation::Show(int type, const std::string &info) {
    ...
    while (p != -1) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (type == 4) {
                //todo: judge whether info is in Keyword.
            }
            if (flag) {
